...
export class UsuariosService {
  ...
  async criarUsuario(dto: CreateUsuarioDto) {
    ...
    if (typeof rawPassword === 'string') {
      if (rawPassword.length < 8) throw new BadRequestException('Senha deve ter no mínimo 8 caracteres');
  ...
  async alterarSenha(email: string, senhaAtual: string, senhaNova: string) {
    if (senhaNova.length < 8) throw new BadRequestException('Senha deve ter no mínimo 8 caracteres');
