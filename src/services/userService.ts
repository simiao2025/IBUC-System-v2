 ...
 * (Canonical implementation, replaces legacy v2)
 */
export const UserService = {
  /**
   * Transforms backend user format to application AdminUser format
   */
  ...
  /**
   * Confirm password recovery code and set new password
   */
  async confirmPasswordRecovery(data: { email: string; codigo: string; senhaNova: string }): Promise<void> {
   ...
   */
  async listRoles(): Promise<{ value: string; label: string }[]> {
   ...
   */
  async listAccessLevels(): Promise<{ value: string; label: string }[]> {
