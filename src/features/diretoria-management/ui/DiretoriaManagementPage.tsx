...
import { diretoriaApi as DiretoriaService } from '../api/diretoria.api';
import { UserService } from '@/entities/user';
import { useAuth } from '@/entities/user';
import { usePolos } from '@/entities/polo';
import type { AdminUser, AdminRole } from '@/entities/user';
import {
  Crown,
  UserCheck,
  FileText,
...
const reverseCargoMappingGeral: Record<string, string> = {
...
const reverseCargoMappingPolo: Record<string, string> = {
...
const DiretoriaManagementPage: React.FC = () => {
  ...
  // Apenas Diretor Geral e Coordenador Geral (e super_admin) podem gerenciar cargos de diretoria
  const canManageDirectorate = adminRole ? (['diretor_geral', 'coordenador_geral', 'super_admin'] as AdminRole[]).includes(adminRole) : false;
  
  /** @deprecated use canManageDirectorate */
  const canManagePoloDirectorate = canManageDirectorate;
  ...
  useEffect(() => {
    ...
    // Forçar visão de polo para admins de polo
    if (directorateMode !== 'polo') {
      setDirectorateMode('polo');
    }
    if (selectedPoloId !== userPoloId) {
      setSelectedPoloId(userPoloId);
    }
  }, [isPoloScoped, userPoloId, directorateMode, selectedPoloId]);
  ...
  useEffect(() => {
    const carregarUsuariosDoPolo = async () => {
      ...
      try {
        ...
        const users = await UserService.list({ polo_id: selectedPoloId, ativo: true });
  ...
  const carregarDiretorias = async () => {
    try {
      ...
      // Se for admin de polo, carregar obrigatoriamente o polo dele
      if (isPoloScoped) {
        if (!userPoloId) {
          setDirectorate([]);
          return;
        }
        const data = await DiretoriaService.listarDiretoriaPolo(userPoloId, true);
      ...
      // Lógica para admins gerais (podem trocar entre geral e polo)
      if (directorateMode === 'polo') {
        if (!selectedPoloId) {
          setDirectorate([]);
          return;
      ...
      const data = await DiretoriaService.listarDiretoriaGeral({ ativo: true });
      const formattedData = (data as any[]).map((item) => ({
        id: item.id,
        nome_completo: item.usuario?.nome_completo || item.nome_completo || 'N/A',
      ...
      // Evitar alertas invasivos em erros de inicialização se não houver dados
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (directorateMode === 'polo' && !canManageDirectorate) {
      alert('Apenas Diretor Geral / Coordenador Geral pode cadastrar/alterar Diretor e Coordenador dos polos.');
      return;
    }
      ...
      if ((isNewUser || directorateMode === 'geral') && !editingPosition && !finalUsuarioId) {
        ...
        try {
          const userRes = await UserService.create({
            ...
            role: targetRole,
            polo_id: selectedPoloId || undefined,
            password: password,
          });
  ...
  const handleEdit = (position: DirectoratePosition) => {
    if (directorateMode === 'polo' && !canManageDirectorate) {
      alert('Apenas Diretor Geral / Coordenador Geral pode editar cargos da diretoria do polo.');
      return;
    }
    setEditingPosition(position);
    const reverseMapping = directorateMode === 'polo' ? reverseCargoMappingPolo : reverseCargoMappingGeral;
    const cargoFrontend = (reverseMapping[position.cargo] || position.cargo) as string;
    setFormData({
  ...
  const toggleStatus = async (positionId: string) => {
    try {
      ...
      if (directorateMode === 'polo' && !canManageDirectorate) {
        alert('Apenas Diretor Geral / Coordenador Geral pode ativar/desativar cargos da diretoria do polo.');
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <PageHeader
        ...
        actionLabel={canManageDirectorate ? "Adicionar Cargo" : undefined}
        actionIcon={canManageDirectorate ? <Crown className="h-4 w-4" /> : undefined}
        onAction={canManageDirectorate ? () => {
          setIsNewUser(true);
          setShowForm(true);
        } : undefined}
      />
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {directorate.map((position) => {
                      const reverseMapping = directorateMode === 'polo' ? reverseCargoMappingPolo : reverseCargoMappingGeral;
                      const cargoKey = (reverseMapping[position.cargo] || position.cargo) as string;
                      ...
                      return (
                        <tr key={position.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {formatDate(position.data_inicio)}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
