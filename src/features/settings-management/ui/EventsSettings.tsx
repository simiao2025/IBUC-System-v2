...
import { eventApi as EventosService } from '@/features/event-management/api/eventos.api';
...
export const EventsSettings: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        await EventosService.update(editing.id, {
  ...
  const handleDelete = async (evt: Evento) => {
    ...
    try {
      await EventosService.delete(evt.id);
