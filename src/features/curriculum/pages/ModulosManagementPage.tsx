...
import { CalendarModelList } from '@/features/calendar/components/CalendarModelList';
import { CalendarModelDetail } from '@/features/calendar/components/CalendarModelDetail';
import { type CalendarModel } from '@/features/calendar/services/calendarModels.service';
...
type ActiveTab = 'modulos' | 'calendarios';
...
const ModulosManagementPage: React.FC = () => {
  ...
  const [selectedCalendarModel, setSelectedCalendarModel] = useState<CalendarModel | null>(null);

  // State for Modules Tab
  ...
  return (
    <AccessControl requiredModule="settings">
      <div className="min-h-screen bg-gray-50">
        <PageHeader
          title="Gestão de Módulos e Calendários"
          subtitle="Gerencie o currículo, lições e modelos de calendário de aulas"
          showBackButton
          backTo="/admin/dashboard"
          actionLabel={activeTab === 'modulos' ? "Novo módulo" : undefined}
          onAction={activeTab === 'modulos' ? novoModulo : undefined}
        />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          {/* Tab Content: Calendários */}
          {activeTab === 'calendarios' && (
            <div className="max-w-7xl mx-auto">
              {selectedCalendarModel ? (
                <CalendarModelDetail 
                  model={selectedCalendarModel} 
                  onBack={() => setSelectedCalendarModel(null)} 
                />
              ) : (
                <CalendarModelList 
                  onSelectModel={setSelectedCalendarModel}
                />
              )}
            </div>
          )}
