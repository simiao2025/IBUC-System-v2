...
import { attendanceApi as PresencasAPI } from '@/entities/attendance';
...
const AttendanceByStudentManagement: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const records = await PresencasAPI.listByStudent(alunoId, inicio || undefined, fim || undefined);
      setData({ registros: records });

      const totalAulas = records.length;
      const presencasCount = records.filter((p: any) => p.status === 'presente').length;
