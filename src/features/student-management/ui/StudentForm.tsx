...
import type { Aluno, Polo, TipoDocumento, Nivel, Turma } from '@/shared/model/database';
...
const StudentForm: React.FC<StudentFormProps> = ({
  ...
  // ðŸ” DEBUG: Log the complete student object received
  useEffect(() => {
    if (student) {
      console.log('ðŸ” StudentForm - student object received:', JSON.stringify(student, null, 2));
      console.log('ðŸ” StudentForm - endereco type:', typeof student?.endereco, student?.endereco);
      console.log('ðŸ” StudentForm - telefone_responsavel:', (student as any)?.telefone_responsavel);
    }
  }, [student]);
  ...
  const safeString = (value: any, fieldName?: string): string => {
    ...
    if (Array.isArray(value)) {
      console.warn(`âš ï¸ safeString received array for ${fieldName}:`, value);
      return '';
    }
    if (typeof value === 'object') {
      console.warn(`âš ï¸ safeString received object for ${fieldName}:`, value);
  ...
  const [formData, setFormData] = useState({
    ...
    // SaÃºde (Try both flat and nested as backend varies)
  ...
  useEffect(() => {
    if (student?.foto_url) {
      console.log('ðŸ“· Foto do aluno carregada de foto_url:', student.foto_url);
  ...
  useEffect(() => {
    if (student?.id) {
      const carregarDocumentos = async () => {
        try {
          const response = await DocumentAPI.listByStudent(student.id) as any;
          console.log('ðŸ“„ Resposta da API de documentos:', response);
          ...
          const docsFormatados = documentos.map((doc: any) => {
            // Extrair tipo do path se nÃ£o estiver definido
          ...
          console.log('ðŸ“ Total de documentos carregados:', docsFormatados.length);

          // Se nÃ£o temos foto_url definida, tentar encontrar nos documentos
          if (!student.foto_url) {
            ...
            if (fotoDoc && fotoDoc.url) {
              console.log('ðŸ“· Foto encontrada nos documentos:', fotoDoc.url);
              setStudentPhoto(fotoDoc.url);
            }
          }
        } catch (error) {
          console.error('âŒ Erro ao carregar documentos do aluno:', error);
        }
      };
  ...
  const handlePhotoUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      // Aqui vocÃª faria o upload da foto
    ...
    if (student?.id) {
      try {
        ...
        for (const file of files) {
          ...
          // Se o arquivo veio do FileUpload, ele jÃ¡ foi enviado para o storage
          // Aqui apenas registramos no banco
          await DocumentAPI.uploadByStudent(student.id, formData, file.tipo);
        }
      ...
      // Modo de criaÃ§Ã£o: apenas mantÃ©m em estado local
      setUploadedFiles(prev => [...prev, ...files]);
    }
  };
  ...
  const validateForm = () => {
    ...
    if (!formData.data_nascimento) newErrors.data_nascimento = 'Data de nascimento Ã© obrigatÃ³ria';
    ...
    if (!formData.polo_id) newErrors.polo_id = 'Polo Ã© obrigatÃ³rio';
    if (!formData.nivel_id) newErrors.nivel_id = 'NÃ­vel Ã© obrigatÃ³rio';

    if (!formData.nome_responsavel.trim()) newErrors.nome_responsavel = 'Nome do responsÃ¡vel Ã© obrigatÃ³rio';
    if (!formData.cpf_responsavel.trim()) newErrors.cpf_responsavel = 'CPF do responsÃ¡vel Ã© obrigatÃ³rio';
    if (!formData.telefone_responsavel.trim()) newErrors.telefone_responsavel = 'Telefone do responsÃ¡vel Ã© obrigatÃ³rio';
    if (!formData.email_responsavel.trim()) newErrors.email_responsavel = 'E-mail do responsÃ¡vel Ã© obrigatÃ³rio';

    if (!formData.alergias.trim()) newErrors.alergias = 'Alergias Ã© obrigatÃ³rio';
    if (!formData.medicacao_continua.trim()) newErrors.medicacao_continua = 'MedicaÃ§Ã£o contÃ­nua Ã© obrigatÃ³rio';
    if (!formData.contato_emergencia_nome.trim()) newErrors.contato_emergencia_nome = 'Contato de emergÃªncia (nome) Ã© obrigatÃ³rio';
    if (!formData.contato_emergencia_telefone.trim()) newErrors.contato_emergencia_telefone = 'Contato de emergÃªncia (telefone) Ã© obrigatÃ³rio';
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-8">
          ...
          <Card className="border-2 border-gray-200">
            ...
            <div className="flex items-center space-x-6">
              ...
              <div className="text-sm text-gray-600">
                ...
                <p>Tamanho mÃ¡ximo: 5MB</p>
                <p>A foto serÃ¡ exibida na ficha do aluno</p>
              </div>
            </div>
          </Card>
          ...
          <Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <Input
                ...
                placeholder="RG sem dÃ­gitos"
              />
              <Input
                label="Ã“rgÃ£o Emissor (RG)"
                name="rg_orgao"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <Select
                ...
                options={turmas.map((t: any) => {
                  const nivelLabel = niveis.find(n => n.id === t.nivel_id)?.nome || 'NÃ­vel';
                  const moduloLabel = t.modulo?.titulo || 'MÃ³dulo';
                  return {
                    value: t.id,
                    label: `${t.nome} [${nivelLabel} - ${moduloLabel}]`
                  };
                })}
                helperText="A turma selecionada define o NÃ­vel e MÃ³dulo do aluno."
              />
              <Select
                label="Status"
          ...
          <Card>
            ...
            <div className="mt-6 border-t pt-6">
              <h4 className="font-medium mb-4">Segundo ResponsÃ¡vel (Opcional)</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <Input
                  label="Nome"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="DoenÃ§as crÃ´nicas ou condiÃ§Ãµes especiais"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-colors resize-none"
                  rows={3}
                />
              </div>
              ...
              <Input
                ...
                placeholder="Hospital em caso de emergÃªncia"
              />
          ...
          <Card>
            ...
            <FileUpload
              ...
              description="Envie os documentos do aluno (certidÃ£o de nascimento, comprovante de residÃªncia, etc.)"
          ...
          <div className="flex justify-end space-x-4 pt-6 border-t">
            ...
            >
              ...
              {student ? 'Salvar AlteraÃ§Ãµes' : 'Cadastrar Aluno'}
            </Button>
          </div>
        </form>
      </div>
