...
// Roles que são considerados "staff" (equipe) do polo
// Revertido para apenas Professor e Auxiliar conforme solicitação
...
const StaffManagementPage: React.FC = () => {
  ...
  // Efeito para inicializar poloId se for admin de polo
  useEffect(() => {
    if (showCreateForm && isPoloScoped && userPoloId && !newStaff.poloId) {
      setNewStaff(prev => ({ ...prev, poloId: userPoloId }));
    }
  }, [showCreateForm, isPoloScoped, userPoloId, newStaff.poloId]);
  ...
  const carregarStaff = async () => {
    try {
      ...
      // Filtrar estritamente pelos papéis de staff
  ...
  const handleCreateStaff = async () => {
    // Validação mais detalhada para ajudar o usuário
    if (!newStaff.name) {
      showFeedback('warning', 'Atenção', 'O nome completo é obrigatório');
      return;
    }
    if (!newStaff.email) {
      showFeedback('warning', 'Atenção', 'O e-mail é obrigatório');
      return;
    }
    if (!newStaff.cpf) {
      showFeedback('warning', 'Atenção', 'O CPF é obrigatório');
      return;
    }
    if (!newStaff.phone) {
      showFeedback('warning', 'Atenção', 'O telefone de contato é obrigatório');
      return;
    }
    if (!newStaff.poloId) {
      showFeedback('warning', 'Atenção', 'Por favor, selecione o Polo');
      return;
    }
    ...
    try {
      ...
      // Mapeamento correto para o backend (snake_case e estrutura de metadata)
      const payload = {
  ...
  const handleUpdateStaff = async () => {
    ...
    try {
      ...
      // Mapeamento para o backend no update
      const payload = {
        ...
        metadata: {
          ...
          hireDate: editingStaff.hireDate || new Date().toISOString()
        }
      };

      await userApi.update(editingStaff.id, payload);
  ...
  const toggleStaffStatus = async (staffId: string) => {
    ...
    try {
      // No backend o campo é 'ativo'
      await userApi.update(staffId, { ativo: !staff.isActive });
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Gestão de Professores e Auxiliares</p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <Card className="w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex space-x-3 mt-6">
              <Button className="flex-1" onClick={handleCreateStaff} disabled={saving}>
                {saving ? (
                  <>
